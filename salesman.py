...
def mutate(ind):
    ...
    L=len(ind)
    for i in xrange(L):#every city has a mutProb chance to be swaped with another
        if random()<mutProb:
...
"""
print "input x point list: e.g. 1,5,3,4"
xpointList=raw_input().split(',')
print "input y point list: e.g. 1,5,3,4"
ypointList=raw_input().split(',')


for i in xrange(len(xpointList)):
    xpointList[i]=int(xpointList[i])
    ypointList[i]=int(ypointList[i])

points=zip(xpointList,ypointList)
"""
